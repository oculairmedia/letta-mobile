...
| GitHub branch protection | Blocks direct pushes, requires `test` + `build-apk-pass` + `shared-multiplatform` + `iroh-probe-gate` to pass, linear history only | Admin override in the UI |
