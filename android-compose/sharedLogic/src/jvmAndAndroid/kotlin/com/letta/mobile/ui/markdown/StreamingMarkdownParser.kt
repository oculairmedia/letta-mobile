...
data class StreamingMarkdownPartition(
    val committedBlocks: List<MarkdownBlock>,
    val activeTail: String,
)

data class StreamingMarkdownRenderPlan(
    val committedBlocks: List<MarkdownBlock>,
    val activeTail: String,
)
...
enum class ParsedTableColumnAlignment {
    Start,
    Center,
    End,
}
 ...
 */
data class MarkdownBlock(val key: String, val text: String)
...
internal fun lineLooksLikeTableSeparator(text: String, start: Int, end: Int): Boolean {
    ...
    while (j < end) {
        ...
        // Optional trailing colon for alignment. The dash run can end exactly
        // at the current streaming boundary, so never index before checking it.
        if (j < end && text[j] == ':') j++
        skipSpaces()
        cellsSeen++
        if (j >= end) break
...
fun buildStreamingMarkdownRenderPlan(
...
fun partitionStreamingMarkdown(text: String): StreamingMarkdownPartition {
    if (text.isEmpty()) {
 ...
 */
fun splitMarkdownBlocks(prefix: String): List<MarkdownBlock> {
 ...
 * content.
 */
fun tailHasOpenBlockFence(tail: String): Boolean {
    var fenceParity = 0
