...
object IdentityAdminHandlers {
    fun register(router: AdminRpcRouter, adminBaseUrl: String) {
        ...
        router.register("identity.get") { p -> id(p, "identity_id")?.let { api.get("identities", it) } ?: jsonError("identity_id required") }
        router.register("project.list") { api.get("projects") }
        router.register("project.get") { p -> id(p, "project_id")?.let { api.get("projects", it) } ?: jsonError("project_id required") }
        router.register("project.create") { p -> api.post("projects", body = p?.toString() ?: "{}") }
        router.register("project.update") { p -> id(p, "project_id")?.let { api.patch("projects", it, body = p.toString()) } ?: jsonError("project_id required") }
        router.register("project.delete") { p -> id(p, "project_id")?.let { api.delete("projects", it) } ?: jsonError("project_id required") }
    }
