...
object ToolAdminHandlers {
    fun register(router: AdminRpcRouter, adminBaseUrl: String) {
        ...
        router.register("tool.get") { p -> id(p, "tool_id")?.let { api.get("tools", it) } ?: adminError("tool_id required") }
        router.register("tool.create") { p -> api.post("tools", body = p?.toString() ?: "{}") }
        router.register("tool.update") { p -> id(p, "tool_id")?.let { api.patch("tools", it, body = p.toString()) } ?: adminError("tool_id required") }
        router.register("tool.delete") { p -> id(p, "tool_id")?.let { api.delete("tools", it) } ?: adminError("tool_id required") }
        router.register("tool.attach") { p ->
            val agentId = id(p, "agent_id") ?: return@register adminError("agent_id required")
            val toolId = id(p, "tool_id") ?: return@register adminError("tool_id required")
        ...
        router.register("block.get") { p -> id(p, "block_id")?.let { api.get("blocks", it) } ?: adminError("block_id required") }
        router.register("block.create") { p -> api.post("blocks", body = p?.toString() ?: "{}") }
        router.register("block.update") { p -> id(p, "block_id")?.let { api.patch("blocks", it, body = p.toString()) } ?: adminError("block_id required") }
        router.register("block.delete") { p -> id(p, "block_id")?.let { api.delete("blocks", it) } ?: adminError("block_id required") }
        router.register("block.attach") { p ->
            val agentId = id(p, "agent_id") ?: return@register adminError("agent_id required")
            val blockId = id(p, "block_id") ?: return@register adminError("block_id required")
        ...
        router.register("block.update_agent") { p ->
            val agentId = id(p, "agent_id") ?: return@register adminError("agent_id required")
            val label = id(p, "label") ?: return@register adminError("label required")
