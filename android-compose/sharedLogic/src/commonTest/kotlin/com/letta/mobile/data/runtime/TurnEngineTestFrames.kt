...
internal class TurnEngineTestFrames(
    ...
    /** `turn_finished` for [run]'s turn number [turn] (turn id `turn-<turn>`). */
    fun turnFinished(turn: Int, run: TestRun): AppServerInboundFrame.TurnFinished {
        seq += 1
...
internal class TurnEngineTestAckingClient(
    ...
    fun emitTurnFinished(turn: Int, run: TestRun) = emit(frames.turnFinished(turn, run))
