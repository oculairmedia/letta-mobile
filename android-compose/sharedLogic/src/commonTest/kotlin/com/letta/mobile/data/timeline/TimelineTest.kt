...
class TimelineTest {
    ...
    private fun confirmed(
        ...
        date: TimelineInstant = timelineNow(),
    ...
    fun `init logs telemetry for position collision`() {
        ...
        assertTrue(positionViolation, "Expected positionViolation telemetry event")
    }
    ...
    fun `init logs telemetry for duplicate otid`() {
        ...
        assertTrue(otidDuplicate, "Expected otidDuplicates telemetry event")
    }
    ...
    // === Sliding-window retention tests ===
    ...
    fun `sliding window updates backfillCursor to the new oldest resident server id`() {
        ...
        assertEquals(t.events.first().let { (it as TimelineEvent.Confirmed).serverId }, t.backfillCursor)
    }
    ...
    fun `unconfirmed local events are never evicted`() {
        ...
        assertNotNull(t.findByOtid("pending-send"))
    }
    ...
    fun `insertOrdered reconcile inserts also respect the sliding window`() {
        ...
        // blow the window back open.
        t = t.insertOrdered(confirmed("missed", 1.0, serverId = "srv-missed"))
        assertTrue(t.events.size <= DEFAULT_MAX_RESIDENT_EVENTS + DEFAULT_EVICT_BUFFER)
    }

    @Test
    fun `small conversations are never evicted`() {
        var t = Timeline("c1")
        repeat(50) { i ->
            t = t.append(confirmed("m$i", (i + 1).toDouble(), serverId = "srv-$i"))
        }
        assertEquals(50, t.events.size)
        assertEquals(0, t.releasedOlderCount)
        assertNull(t.backfillCursor)
    }
}
