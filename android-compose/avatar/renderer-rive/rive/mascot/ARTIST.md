...
## Getting the file into the editor

The source is `scene.rml` (generated by `gen_scene.py`; after your pass, the editor file is the
source of truth and the generator retires). To open it in Rive:

```
rive login                      # once, your Rive account
rive . --once --rev=mascot.rev  # writes an editor file next to the build
# or: rive push                 # creates/updates a file in your Rive workspace
```
