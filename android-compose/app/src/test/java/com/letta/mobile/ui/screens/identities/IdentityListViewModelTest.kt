...
class IdentityListViewModelTest {
    ...
    fun `updateSearchQuery filters identities locally`() = runTest {
        ...
        assertEquals(IdentityId("identity-2"), filtered.first().id)
    }

    @Test
    fun `inspectIdentity loads details`() = runTest {
        viewModel.inspectIdentity(IdentityId("identity-1"))

        val state = viewModel.uiState.value as com.letta.mobile.ui.common.UiState.Success
        assertEquals(IdentityId("identity-1"), state.data.selectedIdentity?.id)
    }
    ...
    fun `updateIdentity updates state without clearing list`() = runTest {
        ...
        viewModel.updateIdentity(
            ...
            params = IdentityUpdateParams(name = "Updated User", blockIds = listOf(BlockId("block-1"))),
        )

        val state = viewModel.uiState.value as com.letta.mobile.ui.common.UiState.Success
        val updated = state.data.identities.first { it.id == IdentityId("identity-1") }
        assertEquals("Updated User", updated.name)
        assertEquals(listOf(BlockId("block-1")), updated.blockIds)
    }

    @Test
    fun `deleteIdentity removes identity from state`() = runTest {
        viewModel.deleteIdentity(IdentityId("identity-1"))

        val state = viewModel.uiState.value as com.letta.mobile.ui.common.UiState.Success
        assertEquals(1, state.data.identities.size)
        assertEquals(IdentityId("identity-2"), state.data.identities.first().id)
    }
    ...
    fun `attachIdentity refreshes selected identity relationships`() = runTest {
        ...
        viewModel.attachIdentity(AgentId("agent-1"), IdentityId("identity-1"))

        val state = viewModel.uiState.value as com.letta.mobile.ui.common.UiState.Success
        assertEquals(listOf(AgentId("agent-1")), state.data.selectedIdentity?.agentIds)
        assertEquals(2, state.data.knownAgents.size)
    }

    @Test
    fun `detachIdentity refreshes selected identity relationships`() = runTest {
        fakeApi.identities[0] = fakeApi.identities[0].copy(agentIds = listOf(AgentId("agent-1")))
        viewModel.inspectIdentity(IdentityId("identity-1"))

        viewModel.detachIdentity(AgentId("agent-1"), IdentityId("identity-1"))

        val state = viewModel.uiState.value as com.letta.mobile.ui.common.UiState.Success
        assertEquals(emptyList<AgentId>(), state.data.selectedIdentity?.agentIds)
    }
