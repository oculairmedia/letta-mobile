...
function registerDeviceActionTool() {
  registerExternalTool({
    ...
    parameters: {
      type: 'object',
      properties: {
        command: { type: 'string', description: 'Device action command, e.g. sensors.summary, sensors.catalog, sensors.sample, hardware.capabilities, intent.dry_run, intent.execute, hardware.flashlight to turn the torch on/off, contacts.read, or calendar.read.' },
