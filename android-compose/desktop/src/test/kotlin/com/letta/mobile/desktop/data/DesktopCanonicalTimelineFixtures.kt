...
/**
 * An empty durable ledger. Every read answers "nothing stored", so any row a test observes had to
 * come from the live overlay or from a write the test itself made.
 */
internal class EmptyTimelineStore : TimelineBoundedStore {
...
/**
 * Fails every call. Opening a presentation must not touch the network, so a test that starts
 * fetching fails loudly here instead of silently passing against a fake page.
 */
internal object NoTimelineTransport : TimelineTransport {
