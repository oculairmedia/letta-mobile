...
class DesktopTimelineBoundedStoreTest {
    ...
    @Test fun toolIndexRestartOverlayRollbackAndScopeIsolation() = runTest {
        val owner = TimelineMessageId("owner")
        store().transaction(scope) {
            nextRevision()
            put(record(1))
            putToolCall(TimelineToolIndexEntry("a", owner, false))
            putToolCall(TimelineToolIndexEntry("b", owner, false))
            putToolCall(TimelineToolIndexEntry("returned-first", null, true))
            setToolSweepGeneration(3)
        }
        store().transaction(scope) {
            nextRevision()
            val returned = requireNotNull(toolCall("returned-first"))
            putToolCall(returned.copy(owner = owner))
            putToolCall(TimelineToolIndexEntry("a", owner, true))
            putToolCall(TimelineToolIndexEntry("c", owner, false))
            assertEquals(listOf("b", "c"), unresolvedTools(null, 2).map { it.callId })
        }
        assertFailsWith<CancellationException> {
            store().transaction(scope) {
                nextRevision()
                putToolCall(TimelineToolIndexEntry("b", owner, true))
        ...
        store().read(scope) {
            ...
            assertEquals(listOf("b"), unresolvedTools(null, 1).map { it.callId })
            assertEquals(listOf("c"), unresolvedTools("b", 1).map { it.callId })
            assertEquals(true, toolCall("returned-first")?.returned)
            assertEquals(owner, toolCall("returned-first")?.owner)
            assertEquals(listOf(1L), metadata(TimelineReadPosition.Tail, 1).rows.map { it.key.order })
        }
        store().read(TimelineScope("other", "conversation")) {
            assertNull(toolCall("b")); assertEquals(0L, toolSweepGeneration())
        }
        assertFailsWith<IllegalArgumentException> {
            store().transaction(scope) { setToolSweepGeneration(2) }
        }
    }
    ...
    @Test fun deferredReferenceSurvivesRestartButNotRevisionChange() = runTest {
        val bytes = ByteArray(32 * 1024) { (it % 251).toByte() }
        val key = record(1).key
        store().transaction(scope) { put(record(1, bytes = bytes)); nextRevision() }
        val reference = store().read(scope) {
            val page = metadata(TimelineReadPosition.Tail, 1)
            val row = page.rows.single()
            TimelineBodyReference(scope, key, row.body, row.contentType, page.revision)
        }
        val chunk = TimelineBoundedReader(store()).readChunk(reference, 0, 1024)
        assertContentEquals(bytes.copyOfRange(0, 1024), chunk.bytes)
        store().transaction(scope) { put(record(1)); nextRevision() }
        assertFailsWith<IllegalStateException> {
            TimelineBoundedReader(store()).readChunk(reference, 0, 1024)
        }
    }

    @Test fun authoritativeIndexRestartNeverOpensDamagedLegacyManifest() = runTest {
        store().transaction(scope) { put(record(1)); nextRevision() }
        val directory = Files.list(root).use { it.findFirst().orElseThrow() }
        // A stale v1 manifest is deliberately unreadable. v2 reads and writes must ignore it.
        Files.write(directory.resolve("active"), ByteArray(81))
        store().read(scope) {
            assertEquals(1L, checkpoint().revision)
            assertEquals(listOf(1L), metadata(TimelineReadPosition.Tail, 1).rows.map { it.key.order })
        }
        store().transaction(scope) { put(record(2)); nextRevision() }
        store().read(scope) {
            assertEquals(2L, checkpoint().revision)
            assertEquals(listOf(1L, 2L), metadata(TimelineReadPosition.Tail, 2).rows.map { it.key.order })
        }
    }
    ...
    @Test fun migrationOversizedBodyDoesNotPublishProgress() = runTest {
        ...
        store().read(scope) {
            assertContentEquals(ByteArray(20), body(metadata(TimelineReadPosition.Tail, 1).rows.single().body, 0, 20))
        }
    }
    ...
    @Test fun substitutedPointersFailClosedButSameRevisionCallbacksResolve() = runTest {
        ...
        store().read(scope) {
            ...
            store().read(scope) {
                assertContentEquals(byteArrayOf(1), body(issued, 0, 1))
            }
        }
    }
    ...
    @Test fun persistent28kRestartAndChangedWriteRemainBounded() = runTest(timeout = kotlin.time.Duration.parse("60m")) {
        ...
        repeat(280) { batch ->
            backend.transaction(scope) {
                nextRevision()
                repeat(100) { put(record((batch * 100 + it).toLong())) }
            }
        }
        val directory = Files.list(root).use { it.findFirst().orElseThrow() }.resolve("v2")
        val index = DesktopPersistentTimelineIndex(directory)
        ...
        val before = Files.list(directory).use { paths -> paths.mapToLong { Files.size(it) }.sum() }
        store().read(scope) {
            assertEquals(280L, checkpoint().revision)
            val tail = metadata(TimelineReadPosition.Tail, 40).rows
            assertEquals((27960L until 28000L).toList(), tail.map { it.key.order })
            val retained = tail.last().body
            store().transaction(scope) {
                nextRevision()
                put(record(-1, "id-27999", byteArrayOf(9)))
                putEvidence("28k", byteArrayOf(42))
            }
            assertContentEquals(byteArrayOf(1), body(retained, 0, 1))
        }
        val after = Files.list(directory).use { paths -> paths.mapToLong { Files.size(it) }.sum() }
        assertTrue(after - before < 1024 * 1024, "28k ordinary write bytes=${after - before}")
        assertTrue(original.roots.order != index.open()?.roots?.order)
        store().read(scope) {
            assertEquals(281L, checkpoint().revision)
            assertEquals(-1L, locate(TimelineMessageId("id-27999"))?.order)
            assertContentEquals(byteArrayOf(42), evidence("28k", 1))
            val first = metadata(TimelineReadPosition.After(record(Long.MIN_VALUE).key), 1).rows.single()
            assertEquals("id-27999", first.key.identity.value)
            assertContentEquals(byteArrayOf(9), body(first.body, 0, 1))
            assertEquals(27998L, metadata(TimelineReadPosition.Tail, 1).rows.single().key.order)
        }
    }
