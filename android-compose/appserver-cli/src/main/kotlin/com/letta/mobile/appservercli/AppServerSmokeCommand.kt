            ...
            withTimeout(timeoutMs.milliseconds) {
                coroutineScope {
                    ...
                    val engine = AppServerTurnEngine(
                        ...
                        requestIdFactory = { "host-cli-${UUID.randomUUID()}" },
                    )
                    ...
                    try {
                        ...
                        engine.runTurn(
                            TurnCommand(
                                backendId = BackendId("app-server-host-cli"),
                                runtimeId = RuntimeId("app-server-host-cli"),
                                agentId = AgentId(agentId),
                                conversationId = ConversationId(conversationId),
                                input = TurnInput.UserMessage(
                                    localMessageId = "host-cli-${UUID.randomUUID()}",
                                    text = message,
                                ),
                            ),
                        ).collect { event ->
