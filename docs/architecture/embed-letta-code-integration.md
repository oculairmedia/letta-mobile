...
- App Server gate, 2026-06-24: embedded `@letta-ai/letta-code` 0.26.1 does not
  expose `letta app-server`. Host 0.27.x exposes App Server but requires Node
  `>=22.19.0`, so Android loopback App Server is blocked until either
...
| `@letta-ai/letta-code@0.27.10` | `>=22.19.0` | Host CLI exposes `app-server` and protocol-v2 assets. | App Server available, but incompatible with the current Android nodejs-mobile 18.20.4 runtime. |
